...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:math="http://exslt.org/math"    xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org"
    version="2.0">
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="node()" mode="insertIdentifier"/>
    </xsl:template>
    
    <xsl:template match="*" mode="insertIdentifier">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:apply-templates select="f:id | f:meta | f:text | f:contained | f:extension | f:modifierExtension | f:identifier"/>
            <xsl:if test="not(f:identifier[f:system = 'https://example.org/nictiz/basic-components/identification-number'])">
                <identifier>
                    <system value="urn:oid:2.16.840.1.113883.2.4.3.11.999.7.6"/>
                    <value value="{uuid:get-uuid(.)}"/> 
                </identifier>
            </xsl:if>
            <xsl:apply-templates select="*[not(self::f:id) and not(self::f:meta) and not(self::f:text) and not(self::f:contained) and not(self::f:extension) and not(self::f:modifierExtension) and not(self::f:identifier)]"/>
        </xsl:copy>
    </xsl:template>
